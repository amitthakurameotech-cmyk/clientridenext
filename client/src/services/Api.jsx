...
export const getAllUserData = async (userId) => {
  try {
    ...
    const [bookingsRes, ridesRes] = await Promise.all([
      API.get("/getbookingdata", {
        headers: { Authorization: `Bearer ${token}` },
      }),
      API.get(`/getdatabyid/${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      }),
    ]);
    ...
    return {
      bookings: taggedBookings,
      rides: taggedRides,
      combined: [...taggedBookings, ...taggedRides],
    };
  } catch (err) {
    throw (
      err.response?.data || { message: "Failed to fetch combined user data" }
    );
  }
};
...
export const getPassengerBookings = async (userId) => {
  try {
    ...
    throw (
      err.response?.data || { message: "Failed to fetch passenger bookings" }
    );
  }
};
...
export const approveOrCancelRequest = async (bookingId, bookingstatus) => {
  try {
    const token = localStorage.getItem("token");
    const res = await API.patch(
      `/approvecanclerequest/${bookingId}`,
      { bookingstatus }, // must be "approved" or "cancelled"
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
